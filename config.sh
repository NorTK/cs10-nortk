...
echo "===> NorTK CONFIG.SH START <==="

# Branding
echo "cs10.nortk.mx" > /etc/hostname

# Set ESP mount options
if [ -f /etc/fstab ]; then
...
# Remove pre-packaged EFI binaries to force KIWI to build a custom GRUB image
# using the modules specified in config.xml.
rm -f /boot/efi/EFI/centos/grubx64.efi
rm -f /boot/efi/EFI/centos/gcdx64.efi
rm -f /boot/efi/EFI/BOOT/BOOTX64.EFI

# Clear machine specific configuration
truncate -s 0 /etc/machine-id

echo "===> NorTK CONFIG.SH END <==="
exit 0
